...
| Загрузка каталога вызывает `get()` без limit на весь набор товаров категории, а пагинация и группировка выполняются в PHP, вместо SQL `LIMIT`/`OFFSET` или keyset-пагинации | `app/Http/Controllers/Catalogue/Queries/Catalogue.php` | high |
...
| В запросе YML условие архива задано через `orWhere` без оборачивающей группировки, из-за чего оператор прецедентности SQL расширяет выборку на все архивные товары вне выбранных категорий | `Price/Index.php::generateYML()` | high |
| Нет `vite.config.*`: отсутствует конфигурация entrypoints, Blade-шаблоны используют `@vite(...)` без чётко определённых точек входа; production build не воспроизводим, все CSS/JS грузятся единым тяжёлым бандлом | корень проекта, `resources/views/layouts/*.blade.php` | high |
...
| Добавить план обслуживания `cookie_consents`: удалить старые pending (TTL cookie pending = TTL cleanup pending = 7 дней), добавить составные индексы по `(status, date)` и `(identifier, status)`, рассмотреть partition по месяцу | Уменьшить размер hot index и ускорить cleanup/analytics | DB size, index size, query time | P0 | Удаление данных согласовать с политикой хранения; partition требует MySQL 8+ и перестройки таблицы |
...
| Переписать YML/Excel exports на `chunkById()` и `response()->streamDownload()`, вынести тяжелую генерацию в queue job с полной структурой класса (`__construct`, `handle`, `ShouldQueue`) | Снизить memory peak и не держать HTTP worker до конца генерации | Memory, request duration, worker availability | P1 | Потребуется хранение временного файла и статус задачи для пользователя |
