...
logger = logging.getLogger("monolith.notification")
    ...
    while True:
        consumer = AIOKafkaConsumer(
            *TOPICS,
            ...
            group_id="monolith-notification-group",
        ...
        try:
            ...
            logger.error("Notification consumer error: %s — retrying in %ds", exc, retry_delay)
...
def start_notification_consumer() -> asyncio.Task:
...
@router.post("/internal/notify", summary="Send a direct notification (internal use)")
async def internal_notify(body: NotifyRequest):
    """Direct notification endpoint for other services, bypassing Kafka."""
    tasks = []
